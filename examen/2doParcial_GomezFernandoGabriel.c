...
void pCarga(FILE* archivo ,sProducto arrayProductos[],int tam)
{
    ...
    for(i=0; i<tam; i++)
    {
        ...
        f_AskForInt(" Stock: ","     Error, reingrese stock: ",&arrayProductos[i].stock,0,100000000);
...
int main()
{
    ...
    if( (fichero=fopen("fichero.csv","r")))
    {
        fichero=fopen("fichero.csv","at");
        pCarga(fichero,arrayProductos,tam);
    }else
    {
         fichero=fopen("fichero.csv","wt");
        pCarga(fichero,arrayProductos,tam);
