...
int askForInt(char* message,int* aux,int j,int min,int max);
...
void requestData(sPersona aPersonas[],int tam)
{
    ...
    for(i=0; i<tam; i++)
    {
        if(askForChar(" Ingrese nombre: ",aux.nombre,i) &&
                askForInt(" Dia de nacimiento: ",&aux.dia,i,1,31) &&
                askForInt(" Mes de nacimiento: ",&aux.mes,i,1,12) &&
                askForInt(" Anio de nacimiento: ",&aux.anio,i,1900,2022))
        {
            aPersonas[i]=aux;
            printf(" \n =============== Carga Exitosa =============== \n");
        }
...
void consultBirthday(sPersona aPersonas[],int tam)
{
    ...
    do
    {
        askForInt(" Ingrese el mes para ver sus cumpleanios o  '0' para salir: ",&aux,0,0,12);
        flag=0;
        system("cls");
        ...
        for(i=0; i<tam; i++)
        {
            if(aux == aPersonas[i].mes){
                printf("\n                  %s  - %d/%d   - Este anio cumple : %d  ",aPersonas[i].nombre,aPersonas[i].dia,aPersonas[i].anio,(2021-aPersonas[i].anio));
                flag=1;
            }
        }
            if(flag==0){
...
int askForInt(char* message,int* aux,int j,int min,int max)
{
    ...
    if(message!=NULL)
    {
        ...
        while( checkRange(aux,min,max)==-1);
    }
    return retorno;
}
